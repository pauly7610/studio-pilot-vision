...
class GovernanceActionIngestion:
    ...
    async def ingest_action(
        ...
        action_entity = GovernanceActionEntity(
            ...
            created_at=datetime.fromisoformat(action_data.get("created_at", datetime.utcnow().isoformat())),
            due_date=datetime.fromisoformat(action_data.get("due_date", datetime.utcnow().isoformat())),
        ...
        await self.client.add_entity(
            ...
            metadata={
                "tier": action_entity.tier.value,
                "status": action_entity.status.value
            }
        )
    ...
    async def update_action_status(
        ...
        if new_status == "completed" and completed_at:
            outcome_id = await self._create_outcome(
                action_id=action_id,
                action_data=updated_properties
            )
    ...
    async def _create_outcome(
        ...
        try:
            ...
            created_at = datetime.fromisoformat(action_data.get("created_at", datetime.utcnow().isoformat()))
            completed_at = datetime.fromisoformat(action_data.get("completed_at", datetime.utcnow().isoformat()))
    ...
    async def ingest_batch_actions(
        self,
