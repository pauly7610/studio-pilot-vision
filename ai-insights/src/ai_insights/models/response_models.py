...
class UnifiedAIResponse(BaseModel):
    ...
    def create_error_response(
        cls, query: str, error_message: str, partial_answer: Optional[str] = None
    ) -> "UnifiedAIResponse":
        """
        ...
        return cls(
            ...
            guardrails=Guardrails(
                answer_type=AnswerType.UNKNOWN, warnings=[error_message], fallback_used=True
            ),
            error=error_message,
        )
    ...
    def create_fallback_response(
        cls, query: str, reason: str, partial_sources: Optional[list[Source]] = None
    ) -> "UnifiedAIResponse":
        """
        ...
        return cls(
            ...
            reasoning_trace=[
                ...
                ReasoningStep(step=2, action="Fallback method used", confidence=0.4),
            ],
            guardrails=Guardrails(
...
class ConfidenceCalculator:
    ...
    def combine_confidences(
        cls, cognee_confidence: float, rag_confidence: float, cognee_weight: float = 0.6
    ) -> float:
        """
