...
class EntityValidator:
    ...
    def generate_stable_id(self, entity_type: str, natural_key: str) -> str:
        """
    ...
    def validate_entity(
        self, entity_id: str, entity_type: str, allow_missing: bool = False
        ...
        try:
            ...
            if entity_data.get("type") != entity_type:
                return (
                    False,
                    None,
                    f"Entity type mismatch: expected {entity_type}, got {entity_data.get('type')}",
                )

            # Cache the result
            self.entity_cache[cache_key] = {"data": entity_data, "cached_at": datetime.utcnow()}
    ...
    def resolve_entity(self, entity_name: str, entity_type: str) -> Optional[str]:
        """
        ...
        # Validate it exists
        is_valid, entity_data, _ = self.validate_entity(stable_id, entity_type, allow_missing=True)
    ...
    def validate_relationship(
        ...
        try:
            ...
            relationships = (
                []
            )  # await self.cognee_client.get_relationships(source_id, relationship_type)
...
class EntityGrounder:
    ...
    async def ground_with_relationships(
        self, entity_id: str, entity_type: str, relationship_types: Optional[list[str]] = None
    ) -> dict[str, Any]:
        """
        ...
        if not is_valid:
            return {"entity": None, "relationships": [], "error": message}

        # Get relationships
        relationships = []
        if relationship_types:
            for rel_type in relationship_types:
                rels = await self.validator.cognee_client.get_relationships(entity_id, rel_type)
