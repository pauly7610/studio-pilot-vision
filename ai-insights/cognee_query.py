...
class CogneeQueryInterface:
    ...
    async def query(
        ...
        if intent in ["blockers", "risks", "revenue_impact"]:
            response["recommended_actions"] = self._generate_recommendations(
                search_results,
                intent
            )
    ...
    def _build_reasoning_trace(
        ...
        trace.append({
            "step": 1,
            "action": f"Parsed query intent as '{intent}'",
            "confidence": 1.0
        })
...
async def execute_query(query_text: str, context: Optional[Dict[str, Any]] = None):
    ...
    for source in result["sources"]:
        print(f"  • {source['entity_type']}: {source['entity_name']} (confidence: {source['confidence']*100:.0f}%)")
