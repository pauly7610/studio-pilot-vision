...
from response_models import (
    UnifiedAIResponse, Source, ReasoningStep, Guardrails,
...
class SharedContext:
    ...
    def get_product_ids(self) -> List[str]:
        ...
        return [
            e["id"] for e in self.grounded_entities
...
class ProductionOrchestrator:
    ...
    async def _cognee_primary_flow(
        ...
        try:
            ...
            return self._format_cognee_response(
                cognee_result, shared_ctx, reasoning_trace
            )
        ...
        try:
            ...
            if cognee_context:
                ...
                reasoning_trace.append(ReasoningStep(
                    ...
                    details={
                        "entities_for_filtering": len(shared_ctx.grounded_entities)
                    },
            ...
            return self._format_rag_response(
                rag_result, shared_ctx, reasoning_trace
            )
    ...
    def _format_rag_response(
        ...
        # Convert RAG sources
        sources = [
            Source(**s) for s in rag_result.get("sources", [])
        ]
        ...
        confidence = ConfidenceCalculator.calculate(
            ...
            entity_grounding=len(shared_ctx.grounded_entities) / max(len(shared_ctx.entity_ids), 1) if shared_ctx.entity_ids else 0.5,
        ...
        return UnifiedAIResponse(
            ...
            answer=answer,
            source_type=SourceType.HYBRID if shared_ctx.historical_context else SourceType.RETRIEVAL,
            confidence=confidence,
            sources=sources,
            reasoning_trace=reasoning_trace,
            guardrails=Guardrails(
    ...
    async def _rag_fallback(
        ...
        if rag_result.get("confidence", 0.0) < self.FALLBACK_THRESHOLD:
            return UnifiedAIResponse.create_error_response(
                query=query,
                error_message="Both Cognee and RAG failed to provide confident answer"
            )
    ...
    async def _cognee_fallback(
        ...
        try:
            ...
            return UnifiedAIResponse.create_error_response(
                query=query,
                error_message=f"Both RAG and Cognee failed: {str(e)}"
            )
    ...
    def _apply_guardrails(
        ...
        if self.CONFIDENCE_THRESHOLD_LOW <= response.confidence.overall < self.CONFIDENCE_THRESHOLD_MEDIUM:
